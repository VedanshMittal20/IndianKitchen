    ...
    <SectionShell variant="void" className="overflow-hidden border-b border-cream/10 py-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <SectionHeader 
          ...
          lightMode
        />
        <div className="flex items-center gap-2 bg-void-light border border-gold/30 px-4 py-2.5 rounded-sm w-fit self-start md:self-auto mb-2 shadow-lg">
          <Star className="w-3.5 h-3.5 text-gold fill-gold" />
          <span className="text-[9px] tracking-[0.2em] uppercase text-cream font-bold font-sans">
      ...
      <div className="relative w-full overflow-visible cursor-grab active:cursor-grabbing">
        ...
        >
          {testimonials.map((t, index) => (
            <div
              key={index}
              className="w-[300px] md:w-[380px] shrink-0 bg-void-light border border-gold/15 p-8 rounded-sm shadow-xl relative overflow-hidden flex flex-col justify-between h-[250px]"
            >
              ...
              <div>
                <div className="flex mb-4 gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-3 h-3 ${i < t.rating ? "text-gold fill-gold" : "text-cream/15"}`}
                    />
                  ))}
                </div>
                
                <p className="font-display italic text-base md:text-lg text-cream/90 leading-relaxed min-h-[90px]">
                  &ldquo;{t.quote}&rdquo;
                </p>
              </div>
              ...
              <div className="border-t border-cream/10 pt-4 flex justify-between items-center">
                <div>
                  <h4 className="text-[9px] tracking-[0.2em] uppercase text-gold font-bold mb-0.5">
                    {t.author}
                  </h4>
                  <p className="text-[8px] tracking-[0.2em] uppercase text-cream/40 font-sans">{t.location}</p>
                </div>
                <span className="text-[8px] tracking-widest text-gold/30 uppercase font-sans">Verified guest</span>
              </div>
            </div>
          ))}
        </motion.div>
      ...
      <div className="w-full text-center mt-6">
        <span className="text-[8px] tracking-[0.25em] text-cream/35 uppercase font-sans">
