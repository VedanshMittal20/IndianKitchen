    ...
    <SectionShell variant="void" className="overflow-hidden border-b border-cream/10 py-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex items-center gap-2 bg-surface-container-low border border-outline-variant/30 px-4 py-2.5 rounded-sm w-fit self-start md:self-auto mb-2 shadow-lg">
          <Star className="w-3.5 h-3.5 text-saffron-gold fill-gold" />
          ...
          {testimonials.map((t, index) => (
            ...
            >
              ...
              <div>
                <div className="flex mb-4 gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-3 h-3 ${i < t.rating ? "text-saffron-gold fill-gold" : "text-on-surface/15"}`}
                    />
                  ))}
                </div>
              ...
              <div className="border-t border-cream/10 pt-4 flex justify-between items-center">
                <div>
                  <h4 className="text-[9px] tracking-[0.2em] uppercase text-saffron-gold font-bold mb-0.5">
                    {t.author}
                  </h4>
                  <p className="text-[8px] tracking-[0.2em] uppercase text-on-surface/40 font-body-md">{t.location}</p>
                </div>
                <span className="text-[8px] tracking-widest text-saffron-gold/30 uppercase font-body-md">Verified guest</span>
              </div>
            </div>
          ))}
        </motion.div>
